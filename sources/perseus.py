"""Perseus Digital Library Beowulf edition interface backed by DuckDB."""
...
def parse_line(raw: str) -> Optional[dict]:
    """
    Parse a single line from the Perseus text file.
...
class Perseus:
    """Interface to the Perseus Digital Library Beowulf edition."""

    def __init__(self, db: Optional[BeoDB] = None) -> None:
        """
        Initialize the Perseus interface.
    ...
    def load_from_txt(self, force: bool = False) -> int:
        """
        Load Perseus Beowulf text from the asset file into DuckDB.
        ...
        if self._db.table_exists(TABLE_NAME):
            if not force:
                logger.info("perseus table already exists, skipping load")
                return self._db.count(TABLE_NAME)
            logger.info("Dropping existing perseus table")
            self._db.drop_table(TABLE_NAME)

        txt_path = get_asset_path(PERSEUS_ASSET)
        logger.info("Loading Perseus Beowulf from TXT", txt_path=str(txt_path))
        ...
        logger.info("Loaded Perseus Beowulf", row_count=row_count, schema=schema)
        return row_count
...
_default_perseus: Optional[Perseus] = None


def get_perseus() -> Perseus:
    """Get the default Perseus instance."""
    global _default_perseus
    if _default_perseus is None:
        _default_perseus = Perseus()
    return _default_perseus


def load(force: bool = False) -> int:
    """Load the Perseus Beowulf text into DuckDB."""
    return get_perseus().load_from_txt(force)


def get_line(line_number: int) -> Optional[dict]:
    """Get a specific line by number."""
    return get_perseus().get_line(line_number)


def get_lines(start: int = 1, end: Optional[int] = None) -> List[dict]:
    """Get a range of lines."""
    return get_perseus().get_lines(start, end)


def search(term: str) -> List[dict]:
    """Search for a term in the Old English text."""
    return get_perseus().search(term)

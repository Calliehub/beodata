"""McMaster University Beowulf edition interface backed by DuckDB."""
...
def parse_line(raw: str) -> Optional[dict]:
    """
    Parse a single line from the McMaster text file.
...
class McMaster:
    """Interface to the McMaster University Beowulf edition."""

    def __init__(self, db: Optional[BeoDB] = None) -> None:
        """
        Initialize the McMaster interface.
    ...
    def load_from_txt(self, force: bool = False) -> int:
        """
        Load McMaster Beowulf text from the asset file into DuckDB.
        ...
        if self._db.table_exists(TABLE_NAME):
            if not force:
                logger.info("mcmaster table already exists, skipping load")
                return self._db.count(TABLE_NAME)
            logger.info("Dropping existing mcmaster table")
            self._db.drop_table(TABLE_NAME)

        txt_path = get_asset_path(MCMASTER_ASSET)
        logger.info("Loading McMaster Beowulf from TXT", txt_path=str(txt_path))
        ...
        logger.info("Loaded McMaster Beowulf", row_count=row_count, schema=schema)
        return row_count
...
_default_mcmaster: Optional[McMaster] = None


def get_mcmaster() -> McMaster:
    """Get the default McMaster instance."""
    global _default_mcmaster
    if _default_mcmaster is None:
        _default_mcmaster = McMaster()
    return _default_mcmaster


def load(force: bool = False) -> int:
    """Load the McMaster Beowulf text into DuckDB."""
    return get_mcmaster().load_from_txt(force)


def get_line(line_number: int) -> Optional[dict]:
    """Get a specific line by number."""
    return get_mcmaster().get_line(line_number)


def get_lines(start: int = 1, end: Optional[int] = None) -> List[dict]:
    """Get a range of lines."""
    return get_mcmaster().get_lines(start, end)


def search(term: str) -> List[dict]:
    """Search for a term in the Old English text."""
    return get_mcmaster().search(term)
